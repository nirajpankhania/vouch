...
- Agent tools hand-write both JSON input_schema (for the SDK) and a zod schema (for validation) over deriving one from the other with `zod-to-json-schema`: schemas are 1–3 fields each, so duplication is cheaper than an unapproved dep and keeps each tool self-documenting.
- `search` uses `git grep` via simple-git over ripgrep or a manual file walk: no new dep, and tracked-files-only is the right scope for "is this used?". Empty result handled in both the resolve and throw paths — git grep's no-match exit is platform-dependent.
- All path inputs jailed to repo root (`path.resolve` + prefix check) and `executeTool` never throws: diff/transcript content is untrusted input, and a string error costs one turn vs a thrown error killing the run.
- Budget enforced by dropping `tools` from the request once `toolCalls >= budget` over sending a "stop now" text mid-tool-use: the API requires every `tool_use` block to get a matching `tool_result`, so a bare text reply there would 400. Withholding tools forces a text verdict cleanly.
- Verdict via JSON-in-prompt + zod validation + one retry + degrade over `output_config.format` structured outputs: the agent-loop skill prescribes it, and it keeps the final-message handling uniform whether or not the model used tools. Degradation = empty hunks + raw text summary, `structured: false` — never crash on model output.
- Anthropic client injected behind a narrow `AgentClient` interface (just `messages.create`) over importing the concrete client in the loop: tests drive a scripted fake with zero network and no API key in CI.
- Verdict factors agent `unrequested` hunks (scope creep alone → `review` → exit 1) over keeping exit codes deterministic-only: the agent layer should be consequential, not advisory; user confirmed 2026-06-14.
...
- `engines.node >= 20` over `>=18` at ship time (cli-ux checklist): Node 18 is EOL; 20 is the floor CI (22) and the dev machine (24) already exceed. `prepublishOnly` runs lint+test+build so a stale `dist/` can never publish.
- README demo uses real captured agent output (the same-diff/opposite-verdict contrast) over a synthetic example or a GIF: authenticity is the whole pitch. A vhs GIF was attempted and abandoned — it only renders cleanly via a WSL toolchain and the output couldn't be verified headlessly; the text block is more convincing anyway.
- AGPL-3.0-only over MIT (from the next release; v1.0.x tags stay MIT irrevocably): copyleft is the posture the author wants for the OSS community. Side effect: license-compatible with imbue-ai/vet (also AGPL).
